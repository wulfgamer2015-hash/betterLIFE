...
// "Bin Vision" — a green AI-scan style filter matching Bin's mascot theme,
// with HUD scanlines and corner brackets baked into the photo.
function captureBinVision() {
  if (!video.videoWidth) return;
  const canvas = document.createElement('canvas');
  const ctx = drawFrameToCanvas(canvas, 'grayscale(1) brightness(1.15) contrast(1.35) sepia(1) hue-rotate(70deg) saturate(4)');
  ...
  ctx.fillText('BIN VISION', m + 12, m + 12);
...
// "Trophy Shot" — a decorative frame + badge for celebrating a scavenger-hunt find.
function captureTrophyShot() {
  if (!video.videoWidth) return;
  const inner = document.createElement('canvas');
  drawFrameToCanvas(inner);
  const iw = inner.width, ih = inner.height;

  const border = Math.round(ih * 0.05);
  const canvas = document.createElement('canvas');
  ...
  grad.addColorStop(0, '#3cd4a0');
  grad.addColorStop(1, '#2fc18a');
  ctx.fillStyle = grad;
  ...
  // inner border line
  ctx.strokeStyle = 'rgba(2,33,26,0.85)';
  ctx.lineWidth = Math.max(2, border * 0.08);
  ctx.strokeRect(border, border, iw, ih);

  // trophy badge, top-left
  const bx = border + 18, by = border + 18, r = Math.round(ih * 0.045);
  ctx.beginPath();
  ctx.arc(bx + r, by + r, r, 0, Math.PI * 2);
  ctx.fillStyle = 'rgba(2,33,26,0.85)';
  ctx.fill();
  ctx.font = `${Math.round(r * 1.3)}px sans-serif`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText('🏆', bx + r, by + r + 1);

  // caption in the bottom frame margin
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.font = `bold ${Math.round(border * 0.42)}px sans-serif`;
  ctx.fillStyle = '#02211a';
  ctx.fillText('FOUND IT!', canvas.width / 2, border + ih + border / 2);
...
function handleCaptureClick() {
  ...
  if (currentMode === 'binvision') { captureBinVision(); return; }
  if (currentMode === 'trophy') { captureTrophyShot(); return; }
  capturePhoto();
}
...
if (!playBtn) console.error('playBtn not found — main menu Play button will not work. Check that index.html and app.js are the same, up-to-date versions (a stale cached copy of one of them is a common cause).');
...
const experimentalSheet = document.getElementById('experimentalSheet');
const closeExperimental = document.getElementById('closeExperimental');

experimentalBtn?.addEventListener('click', () => {
  settingsSheet.setAttribute('aria-hidden', 'true'); // never show both sheets at once
  experimentalSheet.setAttribute('aria-hidden', 'false');
  repositionCheckBtn();
});

closeExperimental?.addEventListener('click', () => {
  experimentalSheet.setAttribute('aria-hidden', 'true');
  settingsSheet.setAttribute('aria-hidden', 'false'); // back to Settings
  repositionCheckBtn();
});
...
// Difficulty tiers (I / II / III / Impossible) determine how obvious the
// objects are. They're no longer a standalone menu — you always pick an
// EVENT first (see EVENT SYSTEM below), then a difficulty within it.
const DIFFICULTY_PROMPT_HINTS = {
  1: 'very common, large, obvious objects that are impossible to miss',
  2: 'common everyday household or personal objects — a fair but doable search',
  3: 'small, easy-to-overlook, or less common objects — genuinely tricky to spot',
  goodluck: 'whimsical, mythical, or joke objects (dragons, unicorns, portals, ghosts, etc.) that are near-impossible to actually find — funny and over-the-top, matching a sarcastic trash can\'s sense of humor'
};
// small always-available fallback pool per tier, used only if AI generation
// isn't possible (no key / local AI / request failure) — keeps the game
// playable even then, just without the event theming.
const DIFFICULTY_FALLBACK_POOLS = {
  1: ['tree', 'chair', 'door', 'window', 'table', 'bed', 'cup', 'book', 'phone', 'shoe', 'lamp', 'plant'],
  2: ['remote control', 'keyboard', 'headphones', 'wallet', 'sunglasses', 'watch', 'hat', 'jacket', 'rug', 'candle', 'mug', 'scissors'],
  3: ['power outlet', 'screwdriver', 'coaster', 'doorknob', 'paperclip', 'rubber band', 'USB drive', 'safety pin', 'thumbtack', 'shoelace', 'bookmark', 'keychain'],
  goodluck: ['dinosaur', 'dragon', 'unicorn', 'ghost', 'alien', 'wizard', 'genie', 'mermaid', 'yeti', 'time machine', 'flying carpet', 'portal to another dimension']
};

// ==================== EVENT SYSTEM ====================
// 5 events are available at any given time:
//  - 1 SEASON event, matching the real-world IRL season (fixed theme)
//  - 2 DAILY events, random theme, refreshes every day
//  - 2 MONTHLY events, random theme, refreshes every month
// The 4 non-season events get a proper, human-friendly name generated by
// Groq (cached so it's not regenerated every time) instead of anything
// random-looking. If there's no key / local AI / generation fails, a
// curated fallback name is used instead — so it never looks broken.
const SEASONS = {
  spring: { label: 'Spring Event', icon: '🌱', theme: 'flowers, blooming plants, rain, gardening tools, pastel-colored things, baby animals, umbrellas, fresh produce' },
  summer: { label: 'Summer Event', icon: '☀️', theme: 'sunglasses, sunscreen, flip-flops, beach gear, ice cream, water bottles, fans, bright/hot-weather items' },
  fall:   { label: 'Fall Event',   icon: '🍂', theme: 'fallen leaves, pumpkins, sweaters, scarves, rakes, backpacks, warm drinks, cozy indoor items' },
  winter: { label: 'Winter Event', icon: '❄️', theme: 'snow gear, gloves, hot drinks, blankets, holiday decorations, candles, heaters, warm clothing' }
};
...
const EVENT_CATEGORIES = [
  { key: 'kitchen', icon: '🍳', theme: 'kitchen tools, cookware, food storage, utensils', fallbackName: 'Kitchen Raid' },
  { key: 'office',  icon: '🖊️', theme: 'desk supplies, stationery, tech accessories, paperwork items', fallbackName: 'Desk Duty' },
  { key: 'sports',  icon: '⚽', theme: 'sports equipment, balls, gym gear, outdoor play items', fallbackName: 'Game Day' },
  { key: 'garage',  icon: '🔧', theme: 'tools, hardware, storage bins, DIY equipment', fallbackName: 'Garage Hunt' },
  { key: 'bathroom',icon: '🧴', theme: 'toiletries, bathroom fixtures, personal care items', fallbackName: 'Bathroom Blitz' },
  { key: 'bedroom', icon: '🛏️', theme: 'bedding, clothing, personal items found in a bedroom', fallbackName: 'Bedroom Sweep' },
  { key: 'tech',    icon: '💻', theme: 'electronics, gadgets, cables, devices', fallbackName: 'Tech Hunt' },
  { key: 'pets',    icon: '🐾', theme: 'pet toys, pet food, pet accessories', fallbackName: 'Pet Patrol' },
  { key: 'art',     icon: '🎨', theme: 'art & craft supplies, drawing tools, creative materials', fallbackName: 'Art Attack' },
  { key: 'travel',  icon: '🧳', theme: 'travel gear, luggage, outdoor/adventure items', fallbackName: 'Packing List' }
];
...
// deterministically picks `count` distinct categories for a given period id
// (e.g. a date string) so everyone sees the same events on the same day/month,
// and they stay stable across reloads until the period rolls over.
function pickCategoriesForPeriod(periodId, count) {
  const pool = EVENT_CATEGORIES.slice();
  const picked = [];
  let s = hashStr(periodId);
...
function getDayId() { return new Date().toISOString().slice(0, 10); }      // e.g. 2026-08-16
function getMonthId() { return new Date().toISOString().slice(0, 7); }     // e.g. 2026-08

const EVENT_NAME_CACHE_STORAGE = 'binEventNameCache';
function loadNameCache() {
  try { return JSON.parse(localStorage.getItem(EVENT_NAME_CACHE_STORAGE)) || {}; }
  catch (e) { return {}; }
}
function saveNameCache(cache) {
  localStorage.setItem(EVENT_NAME_CACHE_STORAGE, JSON.stringify(cache));
}

// Gets (and caches) a good, human-friendly event name from Groq. Falls back
// to a curated name — never a raw/ugly generated string — if AI naming isn't
// available or fails.
async function getEventName(cacheKey, category) {
  const cache = loadNameCache();
  if (cache[cacheKey]) return cache[cacheKey];

  const fallback = category.fallbackName;
  if (currentProvider === 'local') return fallback;
  const key = keyFor('groq');
  if (!key) return fallback;

  try {
    const model = modelFor('groq');
    const prompt = `Come up with a short, catchy 2-4 word event name (like a fun game-mode title) for a photo scavenger hunt themed around: ${category.theme}. Return ONLY the name itself — no quotes, no punctuation besides spaces, nothing else.`;
    const { reply } = await callGroq(key, model, [{ role: 'user', content: prompt }]);
    let name = reply.replace(/["'.\n]/g, '').trim();
    // sanity check: reject anything that doesn't look like a real short name
    if (!name || name.length > 40 || name.length < 3 || !/^[a-zA-Z0-9 &'’\-!]+$/.test(name)) {
      name = fallback;
    }
    cache[cacheKey] = name;
    saveNameCache(cache);
    ...
    console.warn('Event naming failed, using fallback name', e);
    return fallback;
  }
}

// Builds today's full list of 5 events: 1 season + 2 daily + 2 monthly.
async function buildTodaysEvents() {
  const season = getCurrentSeason();
  const seasonInfo = SEASONS[season];
  const dayId = getDayId();
  const monthId = getMonthId();

  const dailyCats = pickCategoriesForPeriod('daily:' + dayId, 2);
  const monthlyCats = pickCategoriesForPeriod('monthly:' + monthId, 2);

  const events = [];
  events.push({ id: 'season', icon: seasonInfo.icon, name: seasonInfo.label, theme: seasonInfo.theme, kind: 'season' });

  for (const cat of dailyCats) {
    const name = await getEventName('daily:' + dayId + ':' + cat.key, cat);
    events.push({ id: 'daily:' + cat.key, icon: cat.icon, name, theme: cat.theme, kind: 'daily' });
  }
  for (const cat of monthlyCats) {
    const name = await getEventName('monthly:' + monthId + ':' + cat.key, cat);
    events.push({ id: 'monthly:' + cat.key, icon: cat.icon, name, theme: cat.theme, kind: 'monthly' });
  }
  return events;
}

// Generates a fresh list of scavenger-hunt objects via the AI, themed to the
// chosen event, instead of pulling from a fixed premade list — so every task
// run is different. Falls back to a static pool if there's no key, local AI
// is active, or generation fails for any reason.
async function generateTaskObjects(difficulty, event) {
  const fallback = () => shuffle(DIFFICULTY_FALLBACK_POOLS[difficulty] || DIFFICULTY_FALLBACK_POOLS[2]);

  if (currentProvider === 'local') return fallback();
  const key = keyFor('groq');
  if (!key) return fallback();

  const hint = DIFFICULTY_PROMPT_HINTS[difficulty] || DIFFICULTY_PROMPT_HINTS[2];
  const count = 12;

  const prompt = difficulty === 'goodluck'
    ? `Generate a JSON array of exactly ${count} short object names for a photo scavenger hunt game. They must be ${hint}. Return ONLY a raw JSON array of lowercase strings, nothing else — no markdown, no explanation.`
    : `Generate a JSON array of exactly ${count} short object names for a photo scavenger hunt game, themed around: ${event.theme}. They must be ${hint}, and realistically findable in or around a home. Return ONLY a raw JSON array of lowercase strings, nothing else — no markdown, no explanation.`;
  ...
  try {
    ...
    if (list.length < 3) return fallback(); // sanity check — too short to be a real hunt
    return shuffle(list);
  } catch (e) {
    console.warn('Task object generation failed, using fallback pool', e);
    return fallback();
  }
}
...
const tasksEventListRow = document.getElementById('tasksEventListRow');
const tasksDifficultyRow = document.getElementById('tasksDifficultyRow');
const tasksDifficultyEventName = document.getElementById('tasksDifficultyEventName');
const tasksDifficultyBackBtn = document.getElementById('tasksDifficultyBackBtn');
...
let taskState = null; // { eventId, eventName, difficulty, order: [...objects shuffled], foundCount }
try {
...
// Bin narrates each newly revealed target — but only when his trash
// personality is active; every other personality stays silent here.
const TASK_NARRATION_LINES = {
  1: ["Very easy. 99.99% of people find this immediately.", "Piece of cake. Almost everyone spots this one right away.", "Barely a challenge. You'll find this in seconds."],
  2: ["Moderate difficulty. Most people find this eventually.", "Not too bad, but keep your eyes open.", "A fair bit of a search, but very doable."],
  3: ["Tricky one. Good luck finding this.", "This one's rough. Not everyone spots it.", "Hard mode earns its name here."],
...
function narrateNewTarget(difficulty, target) {
  if (typeof currentPersonality === 'undefined' || currentPersonality !== 'trash') return;
  const lines = TASK_NARRATION_LINES[difficulty] || TASK_NARRATION_LINES[2];
...
function refreshTasksUi() {
  ...
  tasksEventListRow.hidden = active;
  tasksDifficultyRow.hidden = true; // only shown after picking an event, see showDifficultyPicker()
  ...
  if (active) {
    ...
    if (target) {
      ...
      tasksProgressText.textContent = `${taskState.eventName} — ${taskState.foundCount}/${total} found`;
    } else {
      tasksTargetBadgeText.textContent = 'All done! 🎉';
      tasksTargetBadgeProgress.textContent = `${total}/${total}`;
      tasksProgressText.textContent = `${taskState.eventName} — complete! ${total}/${total}`;
    }
  }
}
refreshTasksUi();

const noKeyModal = document.getElementById('noKeyModal');
const noKeyModalSettingsBtn = document.getElementById('noKeyModalSettingsBtn');
const noKeyModalCloseBtn = document.getElementById('noKeyModalCloseBtn');

function showNoKeyModal() {
  if (!noKeyModal) return;
  noKeyModal.style.display = 'flex';
  noKeyModal.setAttribute('aria-hidden', 'false');
}
function hideNoKeyModal() {
  if (!noKeyModal) return;
  noKeyModal.style.display = 'none';
  noKeyModal.setAttribute('aria-hidden', 'true');
}
noKeyModalCloseBtn?.addEventListener('click', hideNoKeyModal);
noKeyModalSettingsBtn?.addEventListener('click', () => {
  hideNoKeyModal();
  settingsSheet.setAttribute('aria-hidden', 'false');
});

// Renders the 5 event cards into tasksEventListRow (called each time the panel opens)
async function renderEventList() {
  tasksEventListRow.innerHTML = '<div class="eventsLoading">Loading events…</div>';
  const events = await buildTodaysEvents();
  tasksEventListRow.innerHTML = '';
  events.forEach(ev => {
    const card = document.createElement('button');
    card.className = 'eventCard eventCard-' + ev.kind;
    card.innerHTML = `<span class="eventCardIcon">${ev.icon}</span>
      <span class="eventCardName">${ev.name}</span>
      <span class="eventCardKind">${ev.kind === 'season' ? 'Season' : ev.kind === 'daily' ? 'Daily' : 'Monthly'}</span>`;
    card.addEventListener('click', () => showDifficultyPicker(ev));
    tasksEventListRow.appendChild(card);
  });
}

let selectedEvent = null;

function showDifficultyPicker(event) {
  selectedEvent = event;
  tasksEventListRow.hidden = true;
  tasksDifficultyRow.hidden = false;
  if (tasksDifficultyEventName) tasksDifficultyEventName.textContent = `${event.icon} ${event.name}`;
  updateGoodLuckVisibility();
  repositionCheckBtn();
}

tasksDifficultyBackBtn?.addEventListener('click', () => {
  selectedEvent = null;
  tasksDifficultyRow.hidden = true;
  tasksEventListRow.hidden = false;
  repositionCheckBtn();
});

const goodLuckDifficultyBtn = document.getElementById('goodLuckDifficultyBtn');

// "Impossible" difficulty only shows up when Bin's trash personality is active.
...
tasksBtn?.addEventListener('click', async () => {
  // Tasks always needs a Groq key (task-check uses vision, which local AI can't do),
  // regardless of which provider is currently selected in Settings.
  if (!taskState && !keyFor('groq')) {
    showNoKeyModal();
    return;
  }
...
document.querySelectorAll('.difficultyBtn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    document.querySelectorAll('.difficultyBtn').forEach(b => b.disabled = true);

    const order = await generateTaskObjects(difficulty, selectedEvent);

    btn.textContent = original;
    document.querySelectorAll('.difficultyBtn').forEach(b => b.disabled = false);

    taskState = { eventId: selectedEvent.id, eventName: selectedEvent.name, difficulty, order, foundCount: 0 };
    saveTaskState();
    refreshTasksUi();
...
const APP_CAPABILITIES = " Your name is Bin — if asked your name, or who/what you are, say you're Bin. This is true no matter which personality is currently selected, including plainer ones; the personality changes your tone, not your name. " +
  ...
  "The app can ACTUALLY do this and nothing else: take photos, record videos, a Bin Vision mode (a green AI-scan-style filter with HUD scanlines, matching your own trash-can look), " +
  "a Trophy Shot mode (frames a photo with a decorative border and a trophy badge, meant for celebrating a found scavenger-hunt object), " +
  "a Tasks scavenger-hunt game with seasonal/daily/monthly events and difficulty tiers, " +
...
// --- Capture modes: Photo / Bin Vision / Video / Trophy ---
...
function repositionCheckBtn() {
  ...
  const panels = [aiBar, settingsSheet, experimentalSheet, modesBar, tasksBar].filter(
