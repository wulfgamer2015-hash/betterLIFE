...
// progressively smaller/less common/easier to overlook. These static pools are
// now only a FALLBACK (used when there's no API key, local AI is active, or
// generation fails) — normally each task freshly generates its own objects
// via the AI, themed to the current real-world season. See generateTaskObjects().
...
// ---- Seasonal events: the game reads the real-world date and picks the
// matching season automatically (Northern Hemisphere calendar). Each season
// reskins the Tasks panel and flavors the generated objects.
const SEASONS = {
  spring: { label: 'Spring Event', icon: '🌱', sub: "Objects themed to what's blooming outside right now", theme: 'flowers, blooming plants, rain, gardening tools, pastel-colored things, baby animals, umbrellas, fresh produce' },
  summer: { label: 'Summer Event', icon: '☀️', sub: 'Objects themed to sun, heat, and the outdoors', theme: 'sunglasses, sunscreen, flip-flops, beach gear, ice cream, water bottles, fans, bright/hot-weather items' },
  fall:   { label: 'Fall Event',   icon: '🍂', sub: 'Objects themed to autumn and back-to-school season', theme: 'fallen leaves, pumpkins, sweaters, scarves, rakes, backpacks, warm drinks, cozy indoor items' },
  winter: { label: 'Winter Event', icon: '❄️', sub: 'Objects themed to cold weather and the holidays', theme: 'snow gear, gloves, hot drinks, blankets, holiday decorations, candles, heaters, warm clothing' }
};
...
const tasksSeasonBanner = document.getElementById('tasksSeasonBanner');
const tasksSeasonIcon = document.getElementById('tasksSeasonIcon');
const tasksSeasonTitle = document.getElementById('tasksSeasonTitle');
const tasksSeasonSub = document.getElementById('tasksSeasonSub');

function applySeasonTheme() {
  const season = getCurrentSeason();
  const info = SEASONS[season];
  if (!tasksBar) return season;
  tasksBar.classList.remove('season-spring', 'season-summer', 'season-fall', 'season-winter');
  tasksBar.classList.add('season-' + season);
  if (tasksSeasonIcon) tasksSeasonIcon.textContent = info.icon;
  if (tasksSeasonTitle) tasksSeasonTitle.textContent = info.label;
  if (tasksSeasonSub) tasksSeasonSub.textContent = info.sub;
  return season;
}

const DIFFICULTY_PROMPT_HINTS = {
  easy: 'very common, large, obvious objects that are impossible to miss',
  normal: 'common everyday household or personal objects — a fair but doable search',
  hard: 'small, easy-to-overlook, or less common objects — genuinely tricky to spot',
...
// Generates a fresh list of scavenger-hunt objects via the AI instead of
// pulling from a fixed premade list, so every task run is different. Themed
// to the current real-world season. Falls back to the static TASK_POOLS if
// there's no key, local AI is active, or the request/parsing fails for any reason.
async function generateTaskObjects(difficulty, season) {
  const fallback = () => shuffle(TASK_POOLS[difficulty]);

  if (currentProvider === 'local') return fallback();
  const key = keyFor('groq');
  if (!key) return fallback();

  const hint = DIFFICULTY_PROMPT_HINTS[difficulty] || DIFFICULTY_PROMPT_HINTS.normal;
  const seasonInfo = SEASONS[season] || SEASONS.spring;
  const count = (TASK_POOLS[difficulty] || TASK_OBJECTS_NORMAL).length;
  ...
  const prompt = difficulty === 'goodluck'
    ...
    : `Generate a JSON array of exactly ${count} short object names for a photo scavenger hunt game, themed around ${seasonInfo.label.toLowerCase()} (${seasonInfo.theme}). They must be ${hint}, and realistically found in or around a home during this season. Return ONLY a raw JSON array of lowercase strings, nothing else — no markdown, no explanation.`;
